...
(function ($) {
    if($.fn.ajaxForm == undefined) {
		...
		var path = url1[0]+"//"+window.location.hostname+ port + "/health/js/uploadFile/js/jquery.form.js";
		$.getScript(path);
    ...
    $.fn.uploadFile = function (options) {
        ...
        var s = $.extend({
            ...
            showQueueDiv: false,
            statusBarWidth: 170/*500*/,
            dragdropWidth: 170/*500*/,
            showPreview: false,
            previewHeight: "auto",
            previewWidth: 170/*"100%"*/,
        ...
        function setDragDropHandlers(obj, s, ddObj) {
            ...
            ddObj.on('drop', function (e) {
                ...
                if(!s.multiple && files.length > 1) {
                    if(s.showError) {
                    	Message.popup("<div class='" + s.errorClass + "'>" + s.multiDragErrorStr + "</div>",true);
        ...
        function serializeAndUploadFiles(s, obj, files) {
            for(var i = 0; i < files.length; i++) {
                if(!isFileTypeAllowed(obj, s, files[i].name)) {
                    if(s.showError) {
                    	var sMAllowedType = s.allowedTypes;
                    	sMAllowedType = sMAllowedType.replace(/[,]/g,", ");
                    	Message.popup("<div class='" + s.errorClass + "'><b>" + files[i].name + "</b> " + s.extErrorStr + sMAllowedType + "</div>",true);
                    	s.onError(files[i].name,s.errorClass, s.extErrorStr);
                ...
                if(!s.allowDuplicates && isFileDuplicate(obj, files[i].name)) {
                    if(s.showError) {
                    	Message.popup("<div class='" + s.errorClass + "'><b>" + files[i].name + "</b> " + s.duplicateErrorStr + "</div>",true);
                ...
                if(s.maxFileSize != -1 && files[i].size > s.maxFileSize) {
                    if(s.showError) {
                    	Message.popup("<div class='" + s.errorClass + "'><b>" + files[i].name + "</b> " + s.sizeErrorStr + getSizeStr(s.maxFileSize) + "</div>",true);
/*                    	$("<div class='" + s.errorClass + "'><b>" + files[i].name + "</b> " + s.sizeErrorStr + getSizeStr(s.maxFileSize) + "</div>").appendTo(
                ...
                if(s.maxFileCount != -1 && obj.selectedFiles >= s.maxFileCount) {
                    if(s.showError) {
                    	Message.popup("<div class='" + s.errorClass + "'><b>" + files[i].name + "</b> " + s.maxFileCountErrorStr + s.maxFileCount + "</div>",true);
/*                    	$("<div class='" + s.errorClass + "'><b>" + files[i].name + "</b> " + s.maxFileCountErrorStr + s.maxFileCount + "</div>").appendTo(
        ...
        function createCutomInputFile(obj, group, s, uploadLabel) {
            ...
            var fileInputStr = "<input type='file' id='" + fileUploadId + "' title='"+s.title+"' name='" + s.fileName + "' accept='" + s.acceptFiles + "'/>";
            if(s.multiple) {
                if(s.fileName.indexOf("[]") != s.fileName.length - 2) // if it does not endwith
                {
                    s.fileName += "[]";
                }
                fileInputStr = "<input type='file' id='" + fileUploadId + "' title='"+s.title+"'  name='" + s.fileName + "' accept='" + s.acceptFiles + "' multiple/>";
            }
                ...
                if(this.files) //support reading files
                {
                    ...
                    if(!isFileTypeAllowed(obj, s, filenameStr)) {
                        if(s.showError) {
                        	var sMAllowedType = s.allowedTypes;
                        	sMAllowedType = sMAllowedType.replace(/[,]/g,", ");
                        	Message.popup("<div class='" + s.errorClass + "'><b>" + filenameStr + "</b> " + s.extErrorStr + sMAllowedType + "</div>",true);
                        	s.onError(files[i].name,s.errorClass, s.extErrorStr);
/*                        	$("<div class='" + s.errorClass + "'><b>" + filenameStr + "</b> " + s.extErrorStr + s.allowedTypes + "</div>").appendTo(
                ...
                if(feature.fileapi && feature.formdata) //use HTML5 support and split file submission
                {
                    ...
                    if(s.maxFileCount != -1 && (obj.selectedFiles + fileArray.length) > s.maxFileCount) {
                        if(s.showError) {
                        	Message.popup("<div class='" + s.errorClass + "'><b>" + fileList + "</b> " + s.maxFileCountErrorStr + s.maxFileCount + "</div>",true);
            ...
            var options = {
                ...
                success: function (data, message, xhr) {
                    ...
                    if(s.returnType == "json" && $.type(data) == "object" && data.hasOwnProperty(s.customErrorKeyStr)) {
                        ...
                        if(s.showStatusAfterError) {
                            pd.progressDiv.hide();
                            Message.popup("<span class='" + s.errorClass + "'>ERROR: " + msg + "</span>",true);
                ...
                error: function (xhr, status, errMsg) {
                    ...
                    if(xhr.statusText == "abort") //we aborted it
                    {
                        ...
                        if(s.showStatusAfterError) {
                            pd.progressDiv.hide();
                            Message.popup("<span class='" + s.errorClass + "'>ERROR: " + errMsg + "</span>",true);
